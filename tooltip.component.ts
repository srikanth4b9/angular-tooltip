  ...
  setHostStyle(placement: string, disableAutoPlacement: boolean = false): boolean {
      ...
      let scrollTop = window.pageYOffset || document.documentElement.scrollTop || body.scrollTop || 0;
      let scrollLeft = window.pageXOffset || document.documentElement.scrollLeft || body.scrollLeft || 0;
    ...
    if (isCustomPosition) {
      ...
      let topStyle;
      let leftStyle;
      const TOOLTIP_INDENT = 5;


      if (placement === 'top') {
        topStyle = (this.elementPosition.top + scrollTop) - (tooltipHeight + this.tooltipOffset);
      }

      if (placement === 'bottom') {
        topStyle = (this.elementPosition.top + scrollTop) + elementHeight + this.tooltipOffset;
      }

      if (placement === 'top' || placement === 'bottom') {
        leftStyle = scrollLeft + this.elementPosition.left + (elementWidth - tooltipWidth) / 2;
        if(leftStyle + elementWidth > (body.clientWidth || 0) - TOOLTIP_INDENT) {
          this.leftFix = leftStyle + elementWidth - (body.clientWidth || 0) + TOOLTIP_INDENT;
          leftStyle = leftStyle - this.leftFix;
        } else if(leftStyle < TOOLTIP_INDENT) {
          this.rightFix = TOOLTIP_INDENT - leftStyle;
          leftStyle = leftStyle + this.rightFix;
        } else {
          this.leftFix = 0;
          this.rightFix = 0;
        }
      ...
      if (placement === 'left' || placement === 'right') {
        topStyle = (this.elementPosition.top + scrollTop) + elementHeight / 2 - tooltip.clientHeight / 2;
      }
      ...
      if (this.autoPlacement && !disableAutoPlacement) {
        ...
        const bodyHeight = window.innerHeight + scrollTop;
